 ...
 */
export default async function seedTradePrices(container: MedusaContainer) {
  const customerGroupModuleService = container.resolve(Modules.CUSTOMER);
    ...
    for (const product of products) {
      for (const variant of product.variants || []) {
        try {
          ...
          const basePrice = variantPricing[0].price_set.prices?.find(
          ...
          updatedCount += 1;
          console.log(
            `✓ Added trade price for ${product.title} - ${variant.title}: $${(basePrice.amount / 100).toFixed(2)} → $${(tradePrice / 100).toFixed(2)} (${discountPercentage}% off)`
          );
        } catch (error) {
